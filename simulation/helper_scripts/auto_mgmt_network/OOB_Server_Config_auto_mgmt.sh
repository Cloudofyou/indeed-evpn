...
#staging a netq.yml for ci runs
echo " ### Staging a netq.yml file to serve to network ###"
cat <<EOT > /var/www/html/netq.yml
netq-agent:
  port: 31980
...
netq-cli:
  port: 443
  premises: CITC
  server: api.air.netq.cumulusnetworks.com
  netq-user: air@cumulusnetworks.com
  vrf: mgmt
EOT

#staging a netq.yml for the servers they do not use mgmt vrf
echo " ### Staging a netq.yml file to serve to the servers ###"
cat <<EOT > /var/www/html/netq-server.yml
netq-agent:
  port: 31980
  server: 192.168.200.250
netq-cli:
